...
import { tokenize, Token, TokenType, reverseTokenType } from "./lexer";
export default class Parser {
    ...
    public produceAST(sourceCode: string): Program {
        ...
        console.log("Tokens:", this.tokens.map(t => ({ value: t.value, type: reverseTokenType[t.type] || t.type })));
    ...
    private parse_return_statement(): Stmt {
        this.eat(); // eat return
        const value = this.at().type != TokenType.Semicolon ? this.parse_expr() : undefined;
        this.expect(TokenType.Semicolon, "Semicolon (\";\" expected after return statement.");
    ...
    private parse_object_expr(): Expr {
        ...
        while (this.not_eof() && this.at().type !== TokenType.CloseBrace) {
            ...
            if (this.at().type !== TokenType.Identifier && this.at().type !== TokenType.String) {
                throw new Error(`Identifier or String expected as property key in "Object" expression, found ${reverseTokenType[this.at().type] || this.at().type}`);
            }
            const key = this.eat().value;
    
            // Handle shorthand { key }
            ...
            // Expect colon
            this.expect(TokenType.Colon, "Colon (\":\") expected following property key in \"Object\" expression.");
