...
export function eval_call_expr(expr: CallExpr, env: Environment): RuntimeVal {
    ...
    if (expr.caller.kind === 'Identifier' && (expr.caller as Identifier).symbol === 'println') {
        const output = args.map(arg => {
            switch (arg.type) {
                case 'object':
                    const obj = arg as ObjectVal;
                    const props = Array.from(obj.properties.entries())
                        .map(([key, val]) => {
                            if (val.type === 'string') {
                                return `${key}: '${(val as NumberVal | BooleanVal | StringVal).value}'`;
                            }
                            return `${key}: ${(val as NumberVal | BooleanVal | StringVal).value}`;
                        })
                        .join(', ');
                ...
                case 'native-fn':
                case 'array':
                    return `[${arg.type}]`; // Placeholder for complex types
                default:
                    return 'undefined';
            }
        }).join(' ');
